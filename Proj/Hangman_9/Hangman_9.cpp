 ...
 * Created on December 9, 2016, 9:42 AM
 */
...
int  gameMech(string);     //Hangman core game mechanics
int  find(string, char);   //Search for a char in a string var & return its pos
void sortString(string &); //Sort characters in string variable for already
                           //guessed letters (display guessed letters in order)
...
void onePlayer () {
    ...
    if (inDiff) {
        for (int i = 0; i < sizeLst; i++) inDiff >> wordLst[i];
...
//    cout << "Word: " << word << endl;
    ...
    if (scor){
        ...
        if (count < 20){
            ...
            //isNewScore is flag
            if (scor > scores[largInd]) isNewScor = true;
        }
        
        cout << "Your score was " << scor << endl; //display score
        ...
        if (isNewScor){
            ...
            while (name.length() > 10) name.erase(name.length() - 1, 1);
...
int gameMech(string word){
    ...
    do {
        ...
        cout << blnk << "   Guessed: " << guessed << "   SCORE: " << scor
             << "\n";
        ...
        if (correct == false){
            cout << "\nYour guess was wrong. You have "
                 << --lives << " lives left!\n";
            scor = (scor >= 100 ? scor - 100 : 0); //Player loses 100 points
                                                   //from score
        ...
        scor += correct ? 100 * mult * streak : 0; //calculate score
